...
#include <kriso_msgs/DPtoController.h>
...
public:
	...
	{
		return {
				make_handler(&DpCommandPlugin::handle_dp_command)
...
private:
	...
	void handle_dp_command(const mavlink::mavlink_message_t *msg, mavlink::common::msg::KRISO_DP_COMMAND &command)
	{
		...
		data->dp_surge_pgain = command.dp_surge_pgain;
		data->dp_sway_dgain = command.dp_sway_dgain;
		data->dp_sway_pgain = command.dp_sway_pgain;
		data->dp_yaw_dgain = command.dp_yaw_dgain;
		data->dp_yaw_pgain = command.dp_yaw_pgain;
		data->target_wp_lat = command.target_wp_lat;
		data->target_wp_lon = command.target_wp_lon;
		data->targetposition_yaw = command.targetposition_yaw;
...
PLUGINLIB_EXPORT_CLASS(mavros::std_plugins::DpCommandPlugin, mavros::plugin::PluginBase)
